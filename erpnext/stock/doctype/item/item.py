			...
			if vals:
				for key in to_check:
					if cstr(self.get(key)) != cstr(vals.get(key)):
						if self.check_if_linked_document_exists(key):
							frappe.throw(
								_("As there are existing transactions against item {0}, you can not change the value of {1}")
									.format(self.name, frappe.bold(self.meta.get_label(key)))
							)
